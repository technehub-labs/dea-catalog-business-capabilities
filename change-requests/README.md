...
| [CR-DEA-BC-08](CR-DEA-BC-08.md) | Coordinate Technology Management - Resolve N-006 | Proposed (this PR) | — | N-006 (held-unmapped) superseded by N-006R. `dea:capability-technology-management` moves to `strategy-direction × build` (parallels Asset Management and Facility Management). 1 entity YAML + overlay markdown/YAML + provenance strikes (R-004, BC-02 closeout item 5) + change-requests index + CHANGELOG `v1-alpha.2`. Historical research artifacts deliberately untouched. Tag: `v1-alpha.2` to be cut at merge. |
